...
function updateButtonsEnabledState() {
  if (!editor || !parseBtn || !generateBtn) return;
...
var RETAINFORMAT_THEME_KEY = "retainformat_theme";
...
function setTheme(theme) {
  ...
  try {
    localStorage.setItem(RETAINFORMAT_THEME_KEY, theme);
  } catch (e) {}
...
// Rich paste: helpers and clonePasteIntoEditor are in paste-rich.js (loaded before this script).
...
// Restore theme preference (shared with site-theme.js on other pages)
(function initThemeFromStorage() {
  try {
    var t = localStorage.getItem(RETAINFORMAT_THEME_KEY);
